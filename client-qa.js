...
client.on('data', function(data,err) {
    ...
    else
    {
        if(data === 'ASC')
        {
            console.log('Server ASC');
            getJSON();
        }
        if(data === 'DEC')
        {
            console.log('Server DEC');
            client.destroy();
        }
    }
});
client.on('data',function (data,err) {
    ...
    {
        if(data !== 'ASC' && data !== 'DEC')
        {
            console.log('-------------------------');
            if(data.toString() === correct[qNumber - 1].toString())
            {
                console.log(questions[qNumber-1].toString());
                console.log(`Client answer: ` + correct[qNumber - 1].toString());
                console.log(`Server answer: ` + data.toString() + ` ---true`);
            }
            else
            {
                console.log(questions[qNumber-1].toString());
                console.log(`Client answer: ` + correct[qNumber - 1].toString());
                console.log(`Server answer: ` + data.toString() + ` ---false`);
            }
